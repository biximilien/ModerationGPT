module Discord
  class ModerationCommandParser
    ...
    COMMAND_PATTERN = /\A!moderation(?:\s+(?<command>help|watchlist|karma|review))?(?:\s+(?<subcommand>add|remove|reset|history|set|recent|clear|restore))?(?:\s+<@!?(?<user_id>\d+)>)?(?:\s+(?<amount>-?\d+))?\s*\z/i.freeze
