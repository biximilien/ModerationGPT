...
module Harassment
  class ClassificationPipeline
    ...
    def enqueue(message_id:, classifier_version:, enqueued_at: Time.now.utc)
      ...
      existing_job = @classification_jobs.find(message_id:, classifier_version:)
    ...
    def record_success(record)
      ...
      update_job_status(record.message_id, record.classifier_version, ClassificationStatus::CLASSIFIED, updated_at: record.classified_at)

      record
    end

    def record_retryable_failure(message_id:, classifier_version:, error:, retry_at:)
    ...
    def record_terminal_failure(message_id:, classifier_version:, error:)
    ...
    def update_failed_job(message_id:, classifier_version:, status:, error:, available_at:)
      job = fetch_job(message_id:, classifier_version:)
      updated_job = job
    ...
    def update_job_status(message_id, classifier_version, status, updated_at:)
      job = fetch_job(message_id:, classifier_version:)
      @classification_jobs.save(job.with_status(status, updated_at:, available_at: updated_at))
    end

    def fetch_job(message_id:, classifier_version:)
      @classification_jobs.find(message_id:, classifier_version:) ||
        raise(ArgumentError, "classification job not found for message_id=#{message_id} classifier_version=#{ClassifierVersion.build(classifier_version)}")
    end
  end
end
